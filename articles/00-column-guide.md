...
下面是目前优先完成的文章。标题和顺序可能随着项目推进调整；文章发布后，我会把标题替换为对应链接。

| 顺序 | 文章 | 主要问题 | 状态 |
|---:|---|---|---|
| 01 | 为什么物联网平台不应该从 MQTT Topic 开始设计 | 为什么通信地址不能替代业务语义 | 计划中 |
| 02 | W3C WoT 不是一种新协议 | WoT 与具体通信协议是什么关系 | 计划中 |
| 03 | Thing Description 是语义契约，而不是设备配置文件 | TD 应该描述什么、不应该描述什么 | 计划中 |
| 04 | 从 TD 到一次属性读取：ClinkZ-WoT 的完整执行链 | 一次调用如何穿过规划、Binding 和运行时 | 计划中 |
| 05 | 为什么 WoT Runtime 需要预编译执行计划 | 为什么不能在每次调用时重新解释 TD | 计划中 |
| 06 | Logical Plan 与 Binding Artifact：协议中立如何落地 | 哪些决定属于 Core，哪些属于 Binding | 计划中 |
| 07 | Servient 为什么必须成为运行时权威 | 谁拥有路由、生命周期和状态迁移权 | 计划中 |
| 08 | Protocol Binding 为什么不能直接调用 Handler | 如何避免协议组件获得隐藏的执行权 | 计划中 |
| 09 | 为什么 V1 放弃动态插件，选择 Cargo 静态链接 | Rust ABI、异步边界和插件卸载有什么代价 | 计划中 |
| 10 | Generation 如何阻止异步旧结果破坏新状态 | late completion 如何污染新一代资源 | 计划中 |
| 11 | 为什么所有队列、缓存和后台工作都必须有上限 | 如何避免隐藏的无界资源增长 | 计划中 |
| 12 | 我如何用 AI 推进一个复杂 Rust 架构项目 | 如何让 AI 参与工程，而不是只生成代码 | 计划中 |
...
已经熟悉 WoT、主要关注运行时架构，可以从“从 TD 到一次属性读取”开始，再阅读 Logical Plan、Binding Artifact、Servient 和 Protocol Binding 相关内容。
