...
最初接触 W3C WoT 时，很容易把实现任务理解成三部分：

```text
解析 Thing Description
        +
实现 Property / Action / Event API
        +
接入 HTTP、MQTT 或其他协议
```

这些当然都需要，但它们还不足以构成一个完整 Runtime。

真正运行起来以后，引擎必须面对更多问题：一份声明式 TD 怎样形成稳定的执行结构；多个协议组件怎样共享统一语义，又保留各自真实的通信特性；本地 Thing 对外暴露失败时怎样回滚；调用取消以后谁继续持有协议资源；TD 更新以后旧请求和旧订阅怎样退出；同一套行为怎样同时运行在普通服务器和受限设备上。
...
因此，我理解的 WoT 引擎不是“把规范中的名词都做成 Rust struct”，而是：

> 在遵守 WoT 外部语义的前提下，把描述、发现、消费、暴露、协议执行、安全、数据处理和异步生命周期组织成一套权责闭合的运行时。

“完整”也不是指支持的协议越多越好。一个只支持 Zenoh、但对调用、订阅、暴露、取消和清理都有明确所有权的引擎，可能比一个同时接入十种协议、内部却依靠隐式回调和后台任务维持状态的实现更完整。
...
TD 是 WoT 的语义入口，但“支持 TD”绝不只是把 JSON 反序列化成 Rust 类型。
